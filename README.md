...
手動でクローンや `uv sync` をやるのが億劫な人向け。下記のプロンプトを Codex CLI にそのまま貼り付けるだけで導入が完了します。Codex が自律的に進めます。
...
2. リポジトリ内で `uv sync` を実行する（uv 未導入ならインストールから）
...
前提: Windows + PowerShell 7+ + `uv` + `git` + `gh`（オプション）が使える環境。
...
`install.ps1` は `~/.codex/config.toml` に MCP サーバー登録を追記し、`-AppendAgentsRule` フラグが付いていれば `~/.codex/AGENTS.md` にも `config/agents_rule.md` の内容を追記します。
