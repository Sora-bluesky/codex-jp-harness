...
検品ロジックは Codex の LLM 呼び出しの **外側** で走るため、応答トークンには 1 byte も足しません。追加の API コールも外部通信もありません。違反があって Codex が自動で言い直すとき（上の 2.）だけ、1 ターン分の追加トークンが発生します（実測で +15% 程度、違反ゼロなら 0%）。
