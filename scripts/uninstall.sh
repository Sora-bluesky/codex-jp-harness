...
if ! grep -q '^\[mcp_servers\.jp_lint\]' "$CONFIG_PATH"; then
  echo "[ja-output-harness] [mcp_servers.jp_lint] not found; nothing to remove."
else
...
echo ""
echo "[ja-output-harness] Manual step required:"
echo "[ja-output-harness]   1. Edit ~/.codex/AGENTS.md and remove the quality-gate rule block."
echo "[ja-output-harness]   2. Restart Codex (CLI or App)."
