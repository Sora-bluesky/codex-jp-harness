...
本ハーネスは MCP `finalize` ゲートを主軸としつつ、**呼び忘れ検知と次セッションでの再教育**を Codex 0.120.0 以降の hook 機構で補完する。本ドキュメントでは Stop / SessionStart 両 hook の役割・仕組み・インストール手順・フォールバック挙動を解説する。Codex CLI と Codex App は同じ `codex` バイナリを共有するため、本仕様は両者に共通して適用される。

> ⚠️ **Experimental**: hook 機構は Codex 0.120.0 以降のプレビュー実装。リポジトリローカルの `.codex/config.toml` では動かない既知バグ（[Issue #17532](https://github.com/openai/codex/issues/17532)）があるため、本ハーネスは**グローバル `~/.codex/hooks.json` にのみ登録**する方針を採る。
...
**出力例**:
```
[ja-output-harness] 前回セッションで mcp__jp_lint__finalize の呼び忘れを検出しました。内訳: missing-finalize (3回)。日本語応答を返す前に必ず finalize を呼んでください。除外は 4 パターンのみ（コード単独 / 20字以内相槌 / yes-no / 日本語なし）。迷ったら呼ぶ。
```
