...
- `~/.codex/AGENTS.md` から 7.p / 7.q を削除
...
このリポジトリは役目を終えました。OpenAI が Codex CLI に <機能名> を公式実装したため、本ハーネスは不要になりました。
...
公式機能がリリースされても、以下のケースでは即時撤去せず保留する:

- **β 版・実験的機能**の場合 → stable 化を待つ
- **部分的解決**のみ（例: 日本語自然化は入ったが hook は未対応）→ 部分撤去で段階対応
- **OSS コミュニティで代替実装**が優勢 → そちらへの誘導を検討

## 判断記録

撤去を検討した日付と判断内容をここに追記していく:
