...
- dogfooding: 提示された winsmux セッション出力の一部で `preview` / `review` / `drawer` / `terminal` / `desktop` / `code` / `back` が候補として上がることを確認。`SUGGESTION_DICT` 登録済みの語には「プレビュー、確認用」「レビュー」「引き出しパネル」等が自動併記される。
